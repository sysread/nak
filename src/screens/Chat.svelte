<script lang="ts">
  ...
  async function send(): Promise<void> {
    ...
    try {
      ...
      if (needsAutoTitle && loopResult.finalText.length > 0) {
        ...
        // message is also the first. Falls back to `text` only
        // defensively — `messages` is reactive state we've already
        // appended to.
        const seed = messages.find((m) => m.role === 'user')?.content ?? text;
        // Fire-and-forget: don't block the UI on title generation.
        void autoTitle(threadId, seed);
      }
      streamingText = '';
      streamingReasoning = '';
      streamingCitations = null;
