<script lang="ts">
  /*
   * Scanner — a K.I.T.T.-style back-and-forth pulser. Seven thick
   * vertical bars brighten and dim in sequence at full height,
   * sweeping left-to-right and then right-to-left on a loop, after
   * the segmented red grille on the front of K.I.T.T. in the show.
   * Used anywhere the app has a short-lived "something's happening,
   * no useful progress signal yet" moment - the composer's pre-
   * first-token gap, the auto-title generation, etc.
   *
