/**
 * Background topics worker - the Web Worker entry point. Parallels
 * `../summary/worker.ts` in shape; the process work lives in
 * `./loop.ts` and the cross-tab singleton coordination lives in the
 * manager. This file is the message boundary: construct clients,
 * instantiate the TopicsAgent, and drive `runOneCycle` until abort.
 *
...
interface StartMessage {
  ...
  topicsModel: string;
  holderId: string;
  /** Per-thread claim TTL, seconds. One non-streaming Venice call; 120s is generous. */
...
async function runWorker(msg: StartMessage, signal: AbortSignal): Promise<void> {
  ...
  if (sessionError) {
    post({
      type: 'log',
      level: 'error',
      message: `topics worker setSession failed: ${sessionError.message}`,
    });
    return;
  }
  ...
  const coordinator = new LeaseCoordinator(supabase, 'topics', msg.holderId, {
    ttlSeconds: msg.leaseTtlSeconds,
    heartbeatMs: msg.leaseHeartbeatMs,
  });

  const agent = new TopicsAgent(venice, supabase, msg.topicsModel);
      ...
      const ctx: CycleContext = {
        ...
        onLeaseLost: () => {
          post({
            type: 'log',
            level: 'warn',
            message: 'topics lease lost - re-entering polling',
          });
        },
      };
    ...
    runWorker(msg, controller.signal)
      .catch((err: Error) => {
        post({
          type: 'log',
          level: 'error',
          message: `topics worker loop crashed: ${err.message}`,
        });
      })
      .finally(() => {
        workerGlobal.close();
