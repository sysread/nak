 ...
 * re-pin via setSession so all five Supabase clients in the tab
 * share whichever token the main thread just minted. See
 * runWorker's preamble.
 */
...
async function runWorker(msg: StartMessage, signal: AbortSignal): Promise<void> {
  // autoRefreshToken:false because five clients per tab racing to
