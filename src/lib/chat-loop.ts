 ...
 * lock step.
 *
 * Sibling modules: the StreamEvent consumer that maps each event onto
 * the UI handler surface lives in `./chat/stream-events.ts`
 * (`consumeStreamEvents`); the option / result / handler contract types
 * live in `./chat/types.ts`. This file owns the priming + prompt
 * assembly + the single `streamChat` call that feeds the consumer.
 */
