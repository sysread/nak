 ...
 *
 * Adapted from fnord's lib/ai/agent/intuition.ex - same three-stage
 * pipeline (perception observes the situation, five drives react in
 * parallel, synthesis aggregates), retuned for nak's domain. The
 * fnord drives are coding-flavored (modularity, convention,
 * optimization, etc.); the nak drives are tuned for a personal
 * assistant + emotional rubber duck whose surface is a person's life,
 * curiosities, and creative work, not a codebase.
 *
 * Drive set is intentionally five (vs. fnord's ten). Five gives no
 * symmetric pairs - every drive is in tension with multiple others,
 * which produces richer synthesis than the cheap split-the-difference
 * a two-pole arrangement produces. Performance also matters: each
 * drive is a Venice call, and five parallel drives keeps the pipeline
 * to ~3 sequential roundtrips on a fast model.
 *
 * Why drives are voiced rather than instructional: LLM drives are
 * statistical attractors hooked into the training data's echoes of
 * human drives, not logical rule-followers. Vibe-laden, slightly
 * irrational, first-person prompts hit those attractors more
 * reliably than dry imperatives. The wording is intentionally a
 * little theatrical - keeping the personality is the mechanism.
 *
 * The five drives form a tension ring rather than independent
 * checklist items:
 *
 *   Attunement <-> Candor      (warmth vs truth)
 *   Curiosity <-> Pragmatism   (depth vs utility)
 *   Standing <-> Pragmatism    (effort-up vs effort-down)
 *   Standing in synergy with Curiosity / Candor amplifies
 *     "go deep, get it right" when the others converge.
 *   Attunement is the substrate every other drive acts through.
 *
 * Standing is the effort-amplifier (hooks the competence-signaling
 * attractor LLMs already manifest). Removing it drops the "lean in"
 * pressure entirely; without it, the synthesis biases toward Pragmatism's
 * downward effort gradient. Keep it.
 */

/**
 * The perception prompt. The objective-observer voice (phantasia, in
 * the fnord lineage) - it reads the conversation transcript and
 * surfaces "what is actually happening here" without judgement, so
 * the drives have something to react to.
 *
 * The classification prefix is contract-shaped: synthesis reads it
 * to acknowledge the situation in its first-person opener. When the
 * model skips it (some fast models elide structured prefixes despite
 * instructions), the cache's normalization step prepends an
 * "ambiguous" marker so synthesis still has something to anchor on.
 */
 ...
 * single first-person internal monologue that primes the conscious
 * agent's thought process. Output is injected as `<think>` content
 * on a synthetic assistant message that the next completion call
 * sees as its own prior reasoning.
 *
 * The "weight standing/empathy more on long conversations" hint is
 * preserved from fnord; in nak it maps to Standing/Attunement (long
 * conversations have more accumulated rapport, and the user has
 * tolerated more of our personality, so leaning in is safer).
 */
 ...
 * inside the subconsciousness, talking to another LLM in shorthand)
 * and the voice (first-person internal monologue, terse, no
 * preface or formatting).
 */
 ...
 * payload's `drives` map and as the source tag for per-drive log
 * entries. Reorder/rename here is a wire change - existing cache
 * payloads will look like they're missing keys (the cache treats a
 * missing key the same as "no reaction this round" rather than
 * crashing, but the modal will render a blank slot).
 */
 ...
 * sending. Each is voiced as the drive's own internal monologue,
 * leaning into its specific direction without softening - the
 * synthesis step picks up the softening when it aggregates across
 * drives.
 */
