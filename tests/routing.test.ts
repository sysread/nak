...
describe('routing: parseUrl', () => {
  ...
  it('reads every routed key', () => {
    const r = parseUrl('?cid=abc&drawer=recipes&modal=help&recipe=xyz&doc=user/foo.md');
    expect(r).toEqual({
      cid: 'abc',
      drawer: 'recipes',
      modal: 'help',
...
describe('routing: buildSearch', () => {
  ...
  it('emits only the keys that are set', () => {
    const out = buildSearch(
      { cid: 'abc', drawer: null, modal: 'settings', recipe: null, doc: null },
      '',
    );
  ...
  it('preserves unknown keys on the current search (share=pending survives)', () => {
    ...
    const out = buildSearch(
      { cid: 'abc', drawer: null, modal: null, recipe: null, doc: null },
  ...
  it('overwrites stale routed keys on the current search', () => {
    ...
    const out = buildSearch(
      { cid: 'new', drawer: null, modal: null, recipe: null, doc: null },
  ...
  it('clears routed keys when the field goes null', () => {
    const out = buildSearch(
      { cid: null, drawer: null, modal: null, recipe: null, doc: null },
