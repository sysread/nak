...
| 2026-08-25 | local (mise run dev-start) | db6d9a19 | PASS (1-6) | Full re-run at the M6 head, zero forks in DB. All 6 steps verified: (1) trash on user rows, regen on assistant, fork absent on tool-carrying row. (2) hover outlines 5 rows below, clears on leave, count unchanged. (3) destructive delete 8->2, alpha intact. (4) attachment link rows cascade + storage object removed from bucket. (5) empty thread survives with title + composer. (6) disabled mid-send (no user-row trash buttons during stream on the empty thread; post-stream re-enable confirmed). Matches the M0/M1 baseline rows exactly — zero forks = zero behavior change. |
