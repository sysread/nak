...
| ---- | ----------- | ------ | ------ | ----- |
| 2026-08-25 | local (mise run dev-start) | 673be091 | PASS (1-8) | All 8 steps verified. Step 2: fork button (git-branch outline icon, tooltip "Fork the conversation at this message") renders on user rows and settled assistant replies; ABSENT on the tool-carrying assistant row within a tool exchange (only the closing reply offers it). Step 3: hovering the 2nd exchange's closing reply red-outlines rows AFTER it (DOM rows 5-6) via the shared .regen-target channel; hovered row stays clear; leave clears; hovering the last message outlines nothing (tail fork). No greying or deletion at any point. Steps 4-6: fork opens immediately, transcript ends at the clicked reply (position 7, NOT the tail at 9); parent fully intact (7 rows, no outlines, no greying); fork owns 0 rows; fork-point row is parent-owned at position 7. Title carries fraktur-f sigil + ordinal 1. Step 7 (reparent): forking the fork at an inherited row (exchange 1's user message, position 1) correctly sets forked_from_thread_id to the ORIGINAL parent (owner of the clicked row), not the first fork; transcript ends at position 1. Step 8: fork buttons disabled mid-stream (observed during a stuck stream - all action buttons rendered [disabled]); streaming bubble offers no fork button; buttons re-enable after settle. Smoke: drawer fork still works, ordinal increments per fork-point (3 forks from 3 different fork-points each got ordinal 1). Env note: Venice/Mistral connectivity was flaky this session (one stuck stream required manual DB cleanup); the think-leak pattern (think> without <) appeared again on the tool-carrying row. Both pre-existing, not M5 scope. |
