...
| ---- | --- | ------ | ------ | ----- |
| 2026-08-26 | local (mise run dev-start) | 59ea99c1 | PASS (1-5,7), DEFER (6) | Steps 1-5 and 7 pass. (1) Fork created at preceding assistant reply, title "Test successful" (no sigil, markTitle:false), composer pre-populated, 1 draft row. (2) Transcript shows inherited prefix only, no draft card. (3) Original thread untouched (8 messages). (4) Reconnection works. (5) Draft promoted, completion streamed. (7) First message edge case: fresh thread, null parent, no sigil. Step 6 deferred (test ordering). |
| 2026-08-27 | local (mise run dev-start) | 743ed1da | PASS (1-5,7) | Re-run after fixes. Fork created from "Backup test conversation 1" first user message. Composer pre-populated with "Tell me about backup testing." No draft card visible. Reconnection: navigate away and back, composer re-populated from draft row. Edit and send: draft promoted (status=null, content=edited text), completion streamed successfully. |
| 2026-08-26 | local (mise run dev-start) | 59ea99c1 | PASS (1-5,7), DEFER (6) | Steps 1-5 and 7 pass. (1) Fork created at preceding assistant reply, title "Test successful" (no sigil, markTitle:false), composer pre-populated with old user msg text, 1 draft row in DB. (2) Transcript shows inherited prefix only (Hello + Paused), no draft card. (3) Original thread untouched (8 messages, hidden=false). (4) Reconnection: navigate away and back, composer re-populated from draft row. (5) Edit and send: draft promoted (status null, content updated), completion streamed "fork and edit test works", DB confirms. (7) First message edge case: fresh thread with null parent, no sigil, composer has first msg text, 1 draft row. Step 6 (abandoned fork) deferred: test ordering made it ambiguous which "Test successful" thread was selected from the drawer; the core abandon behavior was verified in the edit use case step 4. |
