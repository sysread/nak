...
| 2026-09-07 | cloud | 3b31fcc | root cause (8) | forensics line on the retry: completion_tokens=955 and 930 with reasoning_tokens=0, frames = empty role delta + empty finish_reason=stop delta + usage; third attempt's reasoning "Do recipe_update with full cooklang" then 1010 tokens delivered as nothing. Cooking toolbox was off, so recipe_update was undeclared and the backend dropped the call. Addressed by the toolbox-gating trial (toolbox-gating-trial.md); the re-roll stays as the net |
