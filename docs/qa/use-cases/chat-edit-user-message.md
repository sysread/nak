...
- (8) Without a reload, the old message and its stopped partial fade out and the edited message renders directly above the new reply. Reloading shows the same two rows. Guards against a dead per-thread realtime channel: the edited row reaches the view only through the realtime INSERT echo, and the first turn of a fresh conversation is the busiest window for thread-row echoes (auto-title, topics, updated_at bumps), so a subscription that churns on those echoes fails here first.
