...
8. Edit the first message of a fresh conversation after a stop. Click "New conversation." Send a first message. Click Stop while the reply streams. Click the pencil on that message, choose "Edit," change the text, send, and wait for the reply. Do NOT reload or switch threads yet. Observe the transcript. Then reload.
...
- (8) Without a reload, the old message and its stopped partial fade out and the edited message renders directly above the new reply, in that order. Reloading shows the same two rows. Two mechanisms are under test. The per-thread realtime channel must survive the first turn of a fresh conversation, the busiest window for thread-row echoes (auto-title, topics, updated_at bumps), so a subscription that churns on those echoes fails here first. And the post-commit transcript re-fetch must run after the fade-out: it is what makes the live order deterministic (the echo and the reply's hydration are independent appends) and what lands the row if the echo drops.
...
| 2026-09-11 | cloud session, no browser | (this commit) | NOT RUN (8 added) | Step 8 added for the reported symptom: first message of a new conversation, stopped, edited, resent - the old message faded out and the edited one never appeared until a reload. Root cause traced by reading: the messages realtime effect re-subscribed on every thread-row echo and the same-topic re-create returned a leaving channel, leaving no live subscription. Fix covered by `tests/realtime-subscribe.test.ts`; the browser walkthrough still wants a manual pass. |
| 2026-09-11 | cloud session, no browser | (this commit) | NOT RUN (8 expanded) | Step 8's expected text now covers a second mechanism: the post-commit transcript re-fetch that lands the edited row if its echo drops and makes the live order (edited message above reply) deterministic. Both halves of step 8 remain unexecuted in a browser; the round-1 row above describes the narrower channel-only expectation. |
| 2026-09-11 | local (mise run dev-start, fresh stack) | 405e9fbc | PASS (1, 2, 7-tail, 8) | Step 8 executed in a browser for the first time: fresh conversation, first message sent, Stop clicked mid-stream by an in-page watcher (~1.5s into streaming), destructive edit, resend. Without a reload the old message and the stopped partial faded out and the edited message rendered directly above the completed reply; after a reload the same two rows in the same order. A follow-up send (step-7 tail) kept the full order live: edited, reply, new message, new reply. DB: replacement row pos 1 with the edited text, reply pos 3 (created before the replacement insert, as expected), follow-up rows pos 4/5, old range deleted. Dropdown showed Edit + Fork and edit (1); composer pre-populated, 2 rows carried the regen-target outline (2). Steps 3-6 not re-run this pass (PASS at 59ea99c1 / 743ed1da). |
