...
all your conversations, so it lives here on the (global) tab.
...
opens in its own pop-up rather than on this tab: **click the mood pill**
to see the map and where the current conversation lands on it. The
per-turn predictions that fired for a single message live in the pulse
dropdown on that message instead.
