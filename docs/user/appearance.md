...
Color mode, accent color, and UI style. All three preferences sync
to Supabase so they follow your account across browsers, and are
cached locally so the right look appears instantly on next load.
...
Two looks for the whole interface, picked in **Settings >
Appearance > Style**:

- **Soft** (the default) - rounded corners, hairline card borders,
  and soft drop shadows. The conventional app look.
- **Terminal** - an ANSI-terminal-era rendering: every corner is 90
