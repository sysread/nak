...
On a wide screen the drawer is a permanent column down the left edge
rather than a slide-over. If you are using a mouse, hover the seam
between the drawer and the conversation and the cursor turns into a
resize arrow - drag it to make the column wider or narrower. Your
chosen width is remembered on this device (it lives in the browser, so
it doesn't follow you to another machine) and reapplied the next time
you open Nak. The same drag handle works on the **log drawer** on the
right (see [Chat](./chat.md)).

On a narrower screen - phones, and tablets up to about a 1024px-wide
window - both panels slide over the conversation at a fixed width
instead. The resize handles only appear in the column layout and only
for a mouse pointer, so touch devices never show them.
