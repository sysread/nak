...
- **Images**. The **Smart** tier (Qwen 3.6 Plus) and **Balanced** tier
  (MiniMax M3) are natively vision-capable, so Nak inlines the pixels
  directly and the model sees the image first-hand. **Fast** is text-
  only, so Nak automatically calls a vision sub-model on the main
  model's behalf: the model receives a note listing the attached image
  filenames and calls the built-in `analyze_image` tool to get a text
  description. You do not need to switch tiers to send an image; the
  routing is automatic per tier.
