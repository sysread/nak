...
Search is passage-level on purpose. A long contract is useless as a
single embedding (truncation throws most of it away, and one vector
can't localise "what is the late fee"). The extracted text is split
into chunks at upload, each chunk gets its own embedding, and search
embeds the query once and cosine-ranks every chunk, returning the
best-matching passages grouped by their source document.
...
- `supabase/schema.sql` - the "User Documents (Library)" block defines
  ...
  the `search_document_chunks_by_embedding` search RPC, the private
  `documents` Storage bucket, and the three `storage.objects` RLS
  policies that scope the bucket to each user's `<user_id>/...` prefix.

Data layer:

- `src/lib/supabase.ts` - `Document` / `DocumentChunkHit` interfaces +
  coerce helpers, and the `SupabaseService` methods: `createDocument`,
  ...
  `searchDocumentChunks`, the Storage helpers `uploadDocumentFile` /
...
- `src/lib/tools/doc_search.{schema.,}ts`,
  `doc_list.{schema.,}ts`, `doc_get.{schema.,}ts` - the always-on read
  surfaces (registered in `alwaysOnToolbox`).
...
- **User uploads a file** - `Library.svelte`'s upload form calls
  ...
  sweep (~5 min).
- **Assistant searches** - `doc_search` calls
...
- **Tools** (`docs/dev/tools.md`) - `doc_search` / `doc_list` /
  `doc_get` ride always-on; the `library` toolbox carries the writes.
- **Chat-prompt** (search `LIBRARY_BLOCK`) - tells the model the
  Library exists and when to reach for `doc_search`.

## Gotchas

- **Passage-level search, not document-level.** The unit of retrieval
  is the chunk, not the document. `doc_search` returns passages;
  `doc_get` returns a whole document's text (capped, with a
  `text_truncated` flag pointing the model back to `doc_search` for
  long docs). Don't "simplify" search to one-vector-per-document - that
  reintroduces the truncation problem the chunking exists to solve.
