...
All require Bearer auth (not x402 — for wallet balances, use [`venice-x402`](https://github.com/veniceai/skills/blob/main/skills/venice-x402/SKILL.md)). `GET /billing/balance` and `GET /billing/usage` require an **ADMIN** key — an `INFERENCE` key gets `401`. `GET /billing/usage-analytics` works on any authenticated key (scoped to the account behind the key).
