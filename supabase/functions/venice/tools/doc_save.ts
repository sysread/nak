// doc_save (create/update-merged Library metadata write)
//
// One tool behind an optional `id`: omit it to promote a file the
// user attached to the current conversation into a persistent Library
// document, pass it to update an existing document's title and/or
// description. Consolidates the former doc_create + doc_update. Wire
// schema lives in src/lib/tools/doc_save.schema.ts. Auth: b-strict.
//
// Natural-key dedup: the filename is the natural key against the
// library. A create whose filename exactly matches an existing
// document updates that document's metadata instead of duplicating;
// a near-match is refused with the candidates named (see
// resolveNaturalKeyMatch in _upsert_heuristics.ts).
//
// The extracted body is bound to the uploaded file and is not
// editable here - replacing content would be a re-upload, a user
// action through the UI.
//
// doc_create's promotion path (mirror of src/lib/tools/doc_create.ts):
// find the attachment by filename in the thread, insert the documents
// row, copy the binary from the `attachments` bucket to the
// `documents` bucket if the original is still live, stamp
// extraction_status='done' with the attachment's already-parsed
// extracted_text. The composer ran the Venice text-parser at upload
// time so extracted_text is already on the attachment row; the
// function never re-extracts. An attachment whose binary has been
// reclaimed (90-day expiry; storage_path null) can still be promoted.
//
...
import { requireThreadId, registerTool, type ToolContext } from '../performToolCall.ts';
...
async function doCreate(
  ...
  // The model may be updating a document whose filename it remembers
  // but whose id it forgot; the natural-key heuristic routes an exact
  // match to that document's update path.
  const match = await resolveNaturalKeyMatch({
    query: probeDocuments(ctx),
    keyValue: filename,
    fuzzy: true,
  });
  if (match) {
    ...
    if (args.title !== undefined && args.title !== null) {
      patch.title = (args.title as string).trim().slice(0, MAX_DOCUMENT_TITLE_CHARS);
    }
    patch.description = description.slice(0, MAX_DOCUMENT_DESCRIPTION_CHARS);
    ...
    return { updated: true, document_id: match.id, matched_existing: true };
  }

  return promoteAttachment(args, ctx, filename, description, title);
}
...
export const docSave = {
  ...
  async execute(args: Record<string, unknown>, ctx: ToolContext) {
    ...
    if (id) {
      // Update form: at least one of title/description (description is
      // in the wire required list, but the impl also tolerates a
      // title-only patch).
      if (
      ...
      return doUpdate(id, args, ctx);
    }

    // Create-side required check.
...
/**
 * Promotion path (create form). Find the attachment by filename in
 * the thread, insert the documents row, copy the binary from the
 ...
 */
async function promoteAttachment(
  args: Record<string, unknown>,
  ctx: ToolContext,
  filename: string,
  description: string,
  ...
  const displayTitle =
    typeof title === 'string' && title.trim().length > 0 ? title.trim() : filename;
  ...
  const { data: doc, error: docErr } = await ctx.adminClient
    ...
    .insert({
      user_id: ctx.userId,
      title: (displayTitle ?? filename).slice(0, MAX_DOCUMENT_TITLE_CHARS),
...
// The impl delegates through two resource halves (doCreate/doUpdate)
// that take the full ToolContext; the exported shape above is the
// dispatcher-facing ToolDef.
registerTool(docSave);
