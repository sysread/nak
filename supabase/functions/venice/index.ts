...
import { dispatchMcpRoute } from './mcp/handlers.ts';
...
async function handleStreamFresh(
  ...
  const ctx = await resolveStreamContext(req, body.threadId, { json, userIdFromJwt, requireAdmin });
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // MCP integration routes - dispatched via the handler module so
  // the router stays a thin table. The ctx carries the router
  // helpers (json, userIdFromJwt, requireAdmin, createEdgeLogger)
  // the handlers need without reaching back into this file.
  const mcpResponse = dispatchMcpRoute(route ?? '', req, {
    json,
    userIdFromJwt,
    requireAdmin,
    createEdgeLogger,
