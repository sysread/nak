...
const archives = readdirSync(BACKUPS_DIR)
  ...
  .filter((f) => {
    // New naming: <timestamp>-<target>.tar.gz — only show archives
    // matching the current target so a local backup is never
    // accidentally restored to linked (or vice versa).
    // Old naming: <timestamp>.tar.gz — no target suffix, show for
    // any target (backward compat with pre-target-naming backups).
    if (f.endsWith(`-${target}.tar.gz`)) return true;
    if (f.endsWith('-local.tar.gz') || f.endsWith('-linked.tar.gz')) return false;
    return true;
  })
  .sort()
...
if (archives.length === 0) {
  bail(`No ${target} backup archives found in backups/`, 'Run `mise run backup` first.');
}
